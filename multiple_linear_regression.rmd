...
<ol>
<li> Observational study  $\checkmark$</li>
<li> Experiment </li>
</ol>
...
<li> Yes, revise wording to "Is there an association between beauty and course evaluations?" $\checkmark$</li>
...
<li> The left skewness of the data suggests that the students are less likely to rate the professors highly. $\checkmark$</li>
</ol>
```{r score-dist}
boxplot(evals$score)
```
...
```
\
The relationship between *age* and *bty_avg* looks linear.
...
<li> Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. $\checkmark$</li>
...
```
\
*Check nearly normality by histogram* method 1
```{r}
...
```
\
*Check nearly normality by histogram* method 2
```{r}
...
<li> Tenure Track, Tenured, Teaching $\checkmark$</li>
...
<li> 0.12 points higher than minority professors, all else held constant.$\checkmark$ </li> 
...
drop cls_profssingle:
```{r p-val-select}
m_0 <- lm(score ~ rank + ethnicity + gender + language + age +     cls_perc_eval + cls_students + cls_level + cls_credits + bty_avg + pic_outfit + pic_color, data = evals)
summary(m_0)
```
Yes, the coefficients and significance of the other explanatory variables changed, but only by a small amounts. This indicates that the dropped variable was not collinear with the other explanatory variables.
...
drop gender:
```{r adj-rsq-select}
m_3 <- lm(score ~ rank + ethnicity + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
...
drop language:
```{r}
m_4 <- lm(score ~ rank + ethnicity + gender + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
...
drop age:
```{r}
m_5 <- lm(score ~ rank + ethnicity + gender + language + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
...
drop cle_perc_eval:
```{r}
m_6 <- lm(score ~ rank + ethnicity + gender + language + age 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
...
drop cls_students:
```{r}
m_7 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
              + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_7)$adj.r.squared
```
drop cls_level:
```{r}
m_8 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_profs + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_8)$adj.r.squared
```
drop cls_profs:
```{r}
m_9 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
...
drop cls_credits:
```{r}
m_10 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_10)$adj.r.squared
```
drop bty_avg:
```{r}
m_11 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits
             + pic_outfit + pic_color, data = evals)
summary(m_11)$adj.r.squared
```
drop pic_outfit:
```{r}
m_12 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_color, data = evals)
summary(m_12)$adj.r.squared
```
drop pic_color:
```{r}
m_13 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_profs + cls_credits + bty_avg 
             + pic_outfit, data = evals)
summary(m_13)$adj.r.squared
```
Or we can use the following code to do **backward elimination**: 
(**the lower the AIC value, the better the model**) the following will ignore the traces: 
```{r}
step(m_full, direction = 'backward', trace = FALSE)
```
The regression line equation for the final revised model is:
\
$\widehat{score}$ = 3.9070 + 0.1638*ethnicity + 0.2026*gender - 0.2467*language - 0.0069*age + 0.0049*cls_perc_eval + 0.5172*cls_credits + 0.0467*bty_avg - 0.1139*pic_outfit - 0.1809*pic_color

10. Elimination of which variable from the full model yielded the highest adjusted R-squared? 
<ol>
<li> `bty_avg` </li>
<li> `cls_profs` $\checkmark$</li> 
<li> `cls_students` </li>
<li> `rank` </li>
</ol>
...
No, even if the same professor taught multiple courses, the evaluation scores are independent from each other since each course is different.
...
The professor would be satisfy the following: 
...
* has a high beauty average score

* the color of professor's picture needs to be black&white
...
* some of the criteria are too subjective
