...
We will explore the data using the `dplyr` package and visualize it 
...
To recap: even though we don't know what the full population looks like, we're 99% 
...
Answer: 99% of random samples of size 60 will yield confidence intervals that contain the true average area of houses in Ames, Iowa.
...
Does our confidence interval capture the true average size of houses in 
Ames?
...
data frame where each row represents one bound, as opposed to one interval. 
...
According to the graph, 100% of the confidence intervals include the true population mean, which is very close to our confidence level 99%.
