...
<li> The relationship is positive, linear, and moderately strong. One of the potential outliers is a team with approximately 5520 at bats. $\checkmark$ </li>
...
<ol>
<li> For each additional home run, the model predicts 1.83 more runs, on average. $\checkmark$ </li>
...
<ol>
<li> -15.32 $\checkmark$ </li>
<li> 15.32 </li>
<li> 713 </li>
<li> 5579 </li>
</ol>
```{r residual}
-2789.2429 + 0.6305 * 5579 - 728.3166
...
<li> The residuals show a curved pattern. $\checkmark$ </li> 
...
<li> The residuals are fairly symmetric, with only a slightly longer tail on the right, hence it would be appropriate to deem the the normal distribution of residuals condition met.$\checkmark$ </li>
</ol>
...
```
\
Here I chose the variable `hits`. 
Yes, looks like there's a linear relationship between them.
...
summary(m3)
```
Compare with `runs` and `at_bats`, `hits` has a higher correlation, hence `hits` predicts `runs` better than `at_bats`.
...
<li> hits </li>
<li> wins </li>
<li> batting average $\checkmark$ </li>
</ol>
`wins`:
```{r runs-vs-others}
...
<ol>
<li> on-base plus slugging (`new_obs`) $\checkmark$ </li>
...
new_obs:
**Linearity**: verify linearity with a plot of the residuals vs. fitted(predicted) values
```{r diag}
...
**Nearly normal residuals**: To check this condition, we can look at a histogram
```{r}
ggplot(m6, aes(x = .resid))+
  geom_histogram(binwidth = 25)+
  xlab('Residuals')
```
\
We can see that the distribution of residuals is quite symmetric and nearly normal.

A normal probability plot of the residuals:
```{r}
ggplot(m6, aes(sample = .resid))+
  stat_qq()+
  stat_qq_line(color = 5)
