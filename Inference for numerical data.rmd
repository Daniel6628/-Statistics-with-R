...
```{r habit-weight-box}
boxplot(nc$weight ~ nc$habit, xlab = 'habit', ylab = 'weight')
...
```{r habit-weight-ci}
inference(y = weight, x = habit, data = nc, statistic = 'mean', type = 'ci', method = 'theoretical', order = c('nonsmoker', 'smoker'))
```
...
```
\
The width when the confidence level is 90% is narrower than that of confidence level is 99%.
...
```
We can see that the maximum age for younger mom is 34, the minimum age for mature mom is 35, therefore the cutoff age for younger and mature mothers is 35. 
...
I choose to work with the variable 'weight' and 'gender' I want to know whether the data set provides statistical significant evidence that the weight of the babies, on average, is greater for baby boys than baby girls.
</div>

```{r pick-your-own}
boxplot(nc$weight ~ nc$gender, xlab = 'gender', ylab = 'baby weights')
```
\
According to the two boxplots above, we can see that both the shapes are right skewed; the median weight for baby boys is a little higher than that of for baby girls; the IQR for baby boy's weight as well as variabilities are also slightly wider.

We could conduct the following hypothesis test:
\
\
...
```{r}
inference(y = weight, x = gender, data = nc, statistic = 'mean', type = 'ht', null = 0, alternative = 'less', method = 'theoretical')
```
\
As the p-value quite small, therefore we reject the null hypothesis, which means that the data set provides statistical significant evidence that the weight of the babies, on average, is greater for baby boys than baby girls.
